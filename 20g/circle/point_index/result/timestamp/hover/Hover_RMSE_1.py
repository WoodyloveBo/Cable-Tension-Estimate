...
# ▶ 선 굵기 설정 (원하는 값으로 조절)
PRED_LINEWIDTH = 2.4   # onboard log (pred) 선 굵기
MEAS_LINEWIDTH = 2.4   # tension_gf (meas) 선 굵기
...
plt.rcParams.update({
    ...
    "axes.linewidth": 1.0,
    "grid.alpha": 0.6,
    "lines.linewidth": 1.6,   # 기본값 (개별 plot에서 덮어씀)
...
def moving_average(x, n):
    if n is None or n <= 1: return x
    n = int(n)
    if n <= 1 or n > len(x): return x
...
def plot_window_no_labels(window, pairs_by_cf):
    t0, t1 = window
    duration = t1 - t0   # ex) 36 - 18 = 18 sec

    fig, axes = plt.subplots(3, 1, figsize=(8.1, 6.5), sharex=True)
    ...
    for prefix, pairs in pairs_by_cf.items():
        ...
        # === 절대시간 → 상대시간 변환 ===
        ts_abs = np.array([p[0] for p in pairs_w], dtype=float)
        ts = ts_abs - t0   # 0 ~ (t1 - t0)
        ...
        axes[0].plot(ts, mx, color=color, alpha=0.6, linewidth=MEAS_LINEWIDTH, linestyle='--')

        axes[1].plot(ts, py, color=color, alpha=1.0, linewidth=PRED_LINEWIDTH)
        axes[1].plot(ts, my, color=color, alpha=0.6, linewidth=MEAS_LINEWIDTH, linestyle='--')

        axes[2].plot(ts, pz, color=color, alpha=1.0, linewidth=PRED_LINEWIDTH)
        axes[2].plot(ts, mz, color=color, alpha=0.6, linewidth=MEAS_LINEWIDTH, linestyle='--')

        z_all.extend(pz.tolist())
        z_all.extend(mz.tolist())

    # === X축을 0부터 duration까지로 고정 ===
    for ax in axes:
    ...
    base = f"{os.path.splitext(os.path.basename(BAGFILE))[0]}_0to{duration:.0f}s_comp"
